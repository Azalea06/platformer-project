...
void draw_parallax_background() {
    ...
    float initial_offset      = -(player::get_instance_player().get_player_pos().x * PARALLAX_PLAYER_SCROLLING_SPEED + game_frame * PARALLAX_IDLE_SCROLLING_SPEED);
...
void draw_game_overlay() {
    ...
    // Score
    Vector2 score_dimensions = MeasureTextEx(menu_font, std::to_string(player::get_instance_player().get_total_player_score()).c_str(), ICON_SIZE, 2.0f);
    Vector2 score_position = {GetRenderWidth() - score_dimensions.x - ICON_SIZE, slight_vertical_offset};
    DrawTextEx(menu_font, std::to_string(player::get_instance_player().get_total_player_score()).c_str(), score_position, ICON_SIZE, 2.0f, WHITE);
    ...
    for (size_t row = 0; row < LevelController::get_instance_level().get_current_level().get_rows(); ++row) {
        for (size_t column = 0; column < LevelController::get_instance_level().get_current_level().get_columns(); ++column) {
            ...
            Vector2 pos = {
                    ...
                    (static_cast<float>(column) - player::get_instance_player().get_player_pos().x) * cell_size + horizontal_shift,
...
void draw_player() {
    ...
    Vector2 pos = {
            horizontal_shift,
            player::get_instance_player().get_player_pos().y * cell_size
    };
...
void draw_enemies() {
    ...
    for (auto &enemy : EnemiesController::get_instance().get_enemies()) {
        ...
        Vector2 pos = {
                (enemy.get_pos().x - player::get_instance_player().get_player_pos().x) * cell_size + horizontal_shift,
